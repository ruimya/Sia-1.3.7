...
func (hdb *HostDB) priceAdjustments(entry modules.HostDBEntry, allowance modules.Allowance) float64 {
	...
	_, _, hostCollateral, err := modules.RenterPayoutsPreTax(entry, allowance.Funds.Div64(allowance.Hosts), types.SiacoinPrecision, types.ZeroCurrency, types.ZeroCurrency, allowance.Period, allowance.ExpectedStorage)
	if err != nil {
		hdb.log.Println(err)
		return 0
	}
